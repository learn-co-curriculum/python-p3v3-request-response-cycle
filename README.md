# The Request-Response Cycle : Code-Along

## Learning Goals

- Build and run a Flask application to manipulate and test the structure of a
  request object.
...
Python objects into HTTP responses. There are plenty of strategies to do this by
hand, but Flask makes our lives easier with Werkzeug- you'll find that Flask
...
take `request` objects as arguments. Since every view function needs access to a
`request` object- among other things- Flask manages requests through
**contexts.**

Edit `app.py` to add a simple view that uses a request object:
...
As the name `test_request_context()` suggests, this only provides test data. You
won't use this in your applications, but you might see it in the tests for Phase
4's labs.
...
Edit `app.py` to update the view to display the name of the current app:
...
- `session` is a dictionary object that can be used to hold onto values between
  multiple requests.
...
Every time an application receives a request, it has to decide which view to run
to handle its data. Routes were helpful for defining where to serve our
...
We can view the URL map for ourselves in the Flask shell. Run `flask shell` from
the `server/` directory for this lesson and enter the following:
...
_For more on HTTP request methods, visit [Mozilla HTTP Request
Methods][ moz_http]._

Exit Flask shell and return to the terminal console with the `exit()` function:
...
2. `@app.before_first_request`: runs a function before the first request (but
...
After you restart your server, you should see that our hook has been run and `g`
modified such that `index()` now knows where it lives on your computer. (_Now it
can find its way home if it ever gets lost!_)

![Index page from before with h3 text beneath that says "The path of this
application on the user's device is
/Users/user1/Documents/new-curriculum/intro-to-flask/python-p4-request-response-cycle"](https://curriculum-content.s3.amazonaws.com/python/flask-request-response-2.png)
...
_For more on HTTP status codes, visit the [Mozilla
documentation][moz_http_status]._
...
There are specific cases where your response is meant to do something other than
display HTML in the browser. Two cases are most common: redirects and aborts.
...
- [API - Pallets Projects](https://flask.palletsprojects.com/en/2.3.x/api/)
